...
public:
	...
	OdomTwist() : Node("OdomTwist"){
		...
		odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>("odometry",10,std::bind(&OdomTwist::odomCallback,this,_1));
        status_pub_ = this->create_publisher<std_msgs::msg::Int64>("waypoint_status", 10);
		timer_ = this->create_wall_timer(std::chrono::milliseconds(int(dt * 1000)), std::bind(&OdomTwist::timerCallback, this));
	}
...
private:
	...
	double over_rotate=0.0;//回転ステップで回しすぎた量
    ...
    std_msgs::msg::Int64 status;//走行ステータスの型の宣言(waipoint番号を発信する)
...
void OdomTwist::odomCallback(const nav_msgs::msg::Odometry::SharedPtr odom_msg){
	now_pos.x=odom_msg->pose.pose.position.x;
	now_pos.y=odom_msg->pose.pose.position.y;
	tf2::Quaternion quat_tf;
	tf2::fromMsg(odom_msg->pose.pose.orientation, quat_tf);
	double roll{}, pitch{}, yaw{};
	tf2::Matrix3x3 m(quat_tf);
	m.getRPY(roll, pitch, yaw);
	now_ori=yaw*180/M_PI;
	now_speed_linear=odom_msg->twist.twist.linear.x;
	now_speed_angular=odom_msg->twist.twist.angular.z;
...
void OdomTwist::setParam(){
	...
	over_rotate = (speed_*speed_/2)*180/M_PI;//回しすぎの調整の初期値 : s=(v^2-vo^2)/2a より算出
...
void OdomTwist::step1(){
	double dtw=hypot(set_pos.x-now_pos.x,set_pos.y-now_pos.y);
